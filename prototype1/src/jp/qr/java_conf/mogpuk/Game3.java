...
public class Game3 extends Activity implements OnClickListener  {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		dialog.findViewById(R.id.d_button2).setOnClickListener(new View.OnClickListener() {
			 @Override
				...
				//初回用画像リソース読み込み処理
			 		...
			 		syokai_res[3] =getResources().getIdentifier("crd","drawable", "jp.qr.java_conf.mogpuk");
			 		syokai_res[4] =getResources().getIdentifier("col_cha9","drawable", "jp.qr.java_conf.mogpuk");
				//表示用テキストの分割
			    	String file_result;
			    	String result;
			    	...
			    	is = res.openRawResource(R.raw.txt_game3);
					...
					d_syoukai_btn.setOnClickListener(new View.OnClickListener() {
				            ...
				            public void onClick(View v) {
							 	...
							 	if(count_syokai==3){
							 		d_syoukai_img.setImageResource(R.anim.anim_crd_man);
									AnimationDrawable ani = (AnimationDrawable)d_syoukai_img.getDrawable();
									ani.start();
							 	}
	    ...
	    int cr = getResources().getIdentifier("crd10", "drawable", "jp.qr.java_conf.mogpuk");
        pct.add(cr);
        pct.add(cr);
        
        int crc = getResources().getIdentifier("crd8", "drawable", "jp.qr.java_conf.mogpuk");
        pct.add(crc);
        pct.add(crc);
		...
		hazure1 = getResources().getIdentifier("crd" + (10), "drawable", "jp.qr.java_conf.mogpuk");
		hazure2 = getResources().getIdentifier("crd" + (8), "drawable", "jp.qr.java_conf.mogpuk");
	...
	public void onResume(){
		super.onResume();
		//音楽の読み込み
  		bgm_game = MediaPlayer.create(getBaseContext(), R.raw.bgm_game3);
				...
				if(v==crd[j]){	
					...
					if(stat[j]==0){
						...
						if(hazure_hanbetu==hazure1){
							...
							sp.play( sp_atari , 4.0F , 4.0F , 0 , 0 , 1.0F );
							fukidashi.setText("お手つき回数＋1回\n"+"お手つき回数　残り"+(5-otetuki));
						}
							...
							first[count]=j;	//選んだカードに対応した配列crdの添字を記録
							stat[j]= 1 ;	//選ばれたカードの状態を「1」に変更する
							count++;		//現在選択されているカードの枚数を数える
							if(count==1){
							sp.play( sp_donn , 0.5F , 0.5F , 0 , 0 , 1.0F );
							}
							//2枚カードが選択されているときの処理
							if(count==2){
								a=pct.get(first[0]);	//変数aにfirst[0]に記録されている数字を添字としたpct内の画像を取得させる
								b=pct.get(first[1]);	//変数bにfirst[1]に記録されている数字を添字としたpct内の画像を取得させる
								//表示されている画像が同じなら
								if(a==b){
									stat[first[0]]= 2 ;	//選ばれているカードの状態を2（揃っている）にする 
									stat[first[1]]= 2 ;	
									count=0;			//選択中のカード枚数を0にする
									atari_count++;
									...
									stat[first[0]]= 0 ; //選ばれているカードの状態を0（裏面）にする 
									stat[first[1]]= 0 ;
									count=0;			//選択中のカード枚数を0にする
									sp.play( sp_bubu , 1.0F , 1.0F , 0 , 0 , 1.0F );
									kiyora.setImageResource(R.drawable.nayami);
									otetuki++;
									fukidashi.setText("お手つき回数　残り"+(5-otetuki));
								}
										 ...
										 @Override
								            public void onClick(View v) {
											 if (bgm_game.isPlaying()) {
												...
												//経営終了時刻の計算
												long end_time=time+600*1000;
												//設定がされているなら放置時間をを半分に
											    SharedPreferences po2 = getSharedPreferences("tgl", MODE_PRIVATE);
										        int sta = po2.getInt("status"+8, 0);
										        if(sta==1){
										        	end_time=time+300*1000;
										        }
												//セットする。
												...
												Intent it = new Intent(Game3.this, Go.class);
	...
	class splashHandler implements Runnable {
 		public void run() {
 			...
 			//カードが既に2枚選択されている場合に戻す
			for(int k=0;k<stat.length;k++){
				if(stat[k]==0){
					crd[k].setImageResource(R.drawable.ura);//	裏面を表示
					}
				}
			}
 		}
	public void kouka(int kouka_hanbetu) {
		...
		kekka[0]=rnd.nextInt(5)+10;
		kekka[1]=rnd.nextInt(15)+15;
		kekka[2]=rnd.nextInt(15)+30;
		//技術
		kekka[3]=rnd.nextInt(5)+10;
		kekka[4]=rnd.nextInt(15)+15;
		kekka[5]=rnd.nextInt(15)+30;
		//運
		kekka[6]=rnd.nextInt(15)+15;
		kekka[7]=rnd.nextInt(15)+30;
